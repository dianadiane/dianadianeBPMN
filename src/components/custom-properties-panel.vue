<template>
  <div class="containers" ref="content">
    <el-button
      ...
      >保存为xml</el-button
    >
    <!-- <el-button size="small" @click="saveXmlBt('svg')">保存svg</el-button> -->
...
export default {
  ...
  methods: {
    ...
    saveXmlBt(message) {
      if (message == "xml") {
        this.that.saveDiagram((err, xml) => {
          console.log(xml);
        });
      }
      if (message == "svg") {
        console.log("这里是svg");
        const { svg } = this.bpmnModeler.saveSVG();
        const encodedData = encodeURIComponent(svg);

        let a = document.createElement("a");
        a.download = `diagram.SVG`; //指定下载的文件名
        a.href = `data:application/text/xml;charset=UTF-8,${encodedData}`; //  URL对象
        a.click(); // 模拟点击
        URL.revokeObjectURL(a.href); // 释放URL 对象
      }
      const h = this.$createElement;
      this.Message({
        message: h("p", null, "成功保存"),
        duration: 500,
      });
      let elementRegistry = this.that.bpmnModeler.get("elementRegistry");
      console.log(elementRegistry);
      console.log(this.that);
      console.log(elementRegistry.getAll()); //获取所有元素
    },
  },
  // 计算属性
  computed: {},
