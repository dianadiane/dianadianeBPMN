    ...
    <el-collapse :value="['1', '2','3']">
      <el-collapse-item name="1">
        ...
        <div class="element-item clearfix">
          <span class="flowableAssignee">分配用户：</span>
          <p class="UserTesksAssignee">
            {{ assigneeValue }}
          </p>
        </div>
        <div class="element-item clearfix">
          <span class="flowableCollection">集合(多实例)：</span>
          <p class="UserTesksCollection">
            {{ collectionValue }}
          </p>
        </div>
        <div class="element-item clearfix">
          <span class="flowableElementVariable">元素变量(多实例)：</span>
          <p class="UserTesksElementVariable">
            {{ ElementVariable }}
          </p>
        </div>
      </el-collapse-item>
...
export default {
  ...
  data: () => {
    return {
      ...
      dateValue: "", //日期选择器
      assigneeValue: "", //分配用户
      collectionValue: "", //集合(多实例)
      ElementVariable: "", //元素变量(多实例)
  ...
  //首先创建标签
    ...
    setDefaultProperties() {
      ...
      //分配用户
      // this.assigneeValue =
      //   "${" + ElementObject.$attrs["flowable:assignee"] + "}";
      this.assigneeValue = "${" + "assignee" + "}";
      //集合(多实例)
      // this.collectionValue =
      //   ElementObject.loopCharacteristics.$attrs["flowable:collection"];
      this.collectionValue = "assignee";
      //元素变量(多实例)
      // this.ElementVariable =
      //   ElementObject.loopCharacteristics.$attrs["flowable:elementVariable"];
      this.ElementVariable = "assignee";
      console.log(ElementObject.$attrs);
      //是否有指定
      //传进来的xml是否有指定分配方式
    ...
    assignee(event, type) {
      this.assigneeValue = event;
      this.element.assigneeValue = event;
      this.element[type] = event;
      let properties = {};
      properties[type] = event;
      this.updateProperties(properties);
...
.flowableAssignee,
.flowableCollection,
.flowableElementVariable {
  float: left;
}
.UserTesksAssignee,
.UserTesksCollection,
.UserTesksElementVariable {
  float: left;
  width: 60%;
  margin-left: 20px;
  text-align: left;
